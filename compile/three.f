...
!     Seems to work so far
      do e=1,nelv
        call rone(drdx(1,e),lx1)
        call build_local_1D_laplacian(laply(1,1,e),dph,
     $                                       drdx(1,e),bh,ly1)
        call copy(tmp5(1,1,1,e),ah,lx1*lx1)
      enddo  

      call semhat(ah,bh,ch,dh,zh,dph,jph,bgl,
     $              zglhat,dgl,jgl,nr,wh)
      do e=1,nelv
        call rone(drdx(1,e),lx1)
        call build_local_1D_weak_laplacian(laplx(1,1,e),dph,
     $                                       drdx(1,e),bh,ly1)
      enddo  
    
      call copy(tmp1,laplx,n)
      call copy(tmp2,laply,n)

!     Seems to work so far
      do e=1,nelv
        call rone(tmpx,lx1)
        call set_up_fast_1D_sem_op_again(laplx(1,1,e),0,lx1-1,
     $            .true.,.true.,0.5,0.5,0.5,bh,dph,tmpx,0)
!       Switched indicies        
        call set_up_fast_1D_sem_op_again2(laply(1,1,e),0,lx1-1,
     $            .true.,.true.,0.5,0.5,0.5,bh,dph,tmpx,0)
      ...
      call outpost(tmp1,tmp2,tmp3,tmp4,tmp5,'tmp')    ! laplacian matrix

      return
      end subroutine test_semhat2
!---------------------------------------------------------------------- 
      subroutine build_local_1D_weak_laplacian(ah,dph,drdx,bm,n1)

!     This is the Laplacian Matrix in the weak form
!     ah = DT*B*D        

      implicit none

      integer n1        ! lx1/ly1/lz1
      real ah(n1,n1), dph(n1,n1)
      real drdx(n1), bm(n1)
      ...
      do j=1,n1
        ...
        do k=1,n1
          ah(i,j) = ah(i,j) + 
     $       dph(i,k)*drdx(k)*bm(k)*(1.0/drdx(k))*dph(j,k)*drdx(k)
        enddo
!        ah(i,j) = s
      enddo
      enddo 
      ...
      return
      end subroutine build_local_1D_weak_laplacian
!---------------------------------------------------------------------- 

      subroutine build_local_1D_laplacian(ah,dph,drdx,bm,n1)

!     This is the normal Laplacian Matrix
!     ah = D*B*D 
